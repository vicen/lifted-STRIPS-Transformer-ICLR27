...
Recent work has studied whether transformers trained for next-token prediction acquire world models
\citep{Toshniwal_Wiseman_Livescu_Gimpel_2022,li_iclr,nanda2023linear,world-models2,vafa2025what}
%Recent work has studied the ability of transformers to learn world models from next-token prediction \citep{Toshniwal_Wiseman_Livescu_Gimpel_2022,li_iclr,nanda2023linear,world-models2,vafa2025what}
and generalize to longer sequences
\citep{kazemnejad2023impact,zhou2024algorithms,huang2025formal}.
\citet{sarrof2026verify} study the related task of plan verification in the lifted \strips setting, where the domain is given and transformers must generalize both to longer plans and to larger numbers of objects. 
...
Finally, \citet{nunez2026strips} use transformers to learn propositional \strips models from action sequences for use with off-the-shelf classical planners. %\citep{ghallab:book,geffner:book}.
The input traces contain ground actions over concrete objects, whereas the target model consists of object-independent action schemas.
...
A learned lifted model should therefore generalize not only to traces of different lengths, but also to problem instances with different numbers of objects.
%Therefore, learning such a world model requires generalizing not only to traces of different lengths, but also to problem instances with different numbers of objects.
...
We consider two supervision settings.
Under \emph{applicability supervision}, each trace contains an initial state and an action sequence in which every action is labeled as applicable or inapplicable given the initial state and the preceding actions, and no subsequent states are observed.
Under \emph{final-state supervision}, each trace contains an initial state, an applicable action sequence, and the resulting final state, while intermediate states remain unobserved.
...
Our main contribution is a pair of transformer architectures for learning lifted \strips models, extending \citet{nunez2026strips} from propositional to lifted models. 
The \emph{Lifted \strips (LS) Transformer} incorporates explicit symbolic structure, with parameters mapping directly to \strips preconditions and effects, and is based on a \brasp formulation of lifted \strips trace recognition. 
The \emph{Lifted Stick-Breaking (LB) Transformer} is closer to a standard Transformer, replacing softmax with stick-breaking attention~\citep{tan2025scaling}, without positional encodings, and using randomized object embeddings.
Under applicability supervision, we train the LS Transformer and extract the \strips model directly from its parameters.
The LB Transformer could also be trained in this setting, but its parameters do not map directly to \strips, so we do not extract a symbolic model.
Under final-state supervision, we train both architectures and recover the model through state reconstruction: the transformer predicts the unobserved intermediate states, and a algorithm extracts the lifted \strips model from the reconstructed state-action traces.
Figure~\ref{fig:applicability} illustrates both settings.

We evaluate the LS and LB Transformers on five benchmark planning domains.
Under final-state supervision, both architectures achieve high accuracy, substantially outperforming standard Transformer baselines with softmax attention and positional encodings.
%The LS Transformer achieves perfect training, test, and planning accuracy for every seed across all five domains.
Additionally, both architectures show strong generalization to traces up to ten times longer and, separately, to problem instances with up to ten times more objects than those seen during training.
%they show near-perfect generalization to traces containing up to ten times more actions and objects than those seen during training.
Finally, we extract lifted \strips models from both transformers and use them for planning with off-the-shelf symbolic planners.
...
% The results paragraph currently overstates the empirical evidence. This is the most important issue because it affects scientific accuracy. “Both architectures achieve high accuracy” hides the large seed variance of LS under applicability supervision. “Near-perfect generalization” is too strong for LB, whose object-generalization accuracy in \lgt drops to about \(80\%\). The generalization experiments also vary trace length and number of objects separately and report the best-training seed, not an average over seeds. Finally, “we extract accurate lifted \strips models from both transformers” is not universally true: LB recovers an incorrect \lgt model, and there is also an LS applicability case with extra effects that hurt planning.
...
%    \includegraphics[width=\textwidth]{combined4_A.png}
    \includegraphics[width=\textwidth]{combined6_A.png}

\caption{
\textbf{Overview of the two architectures and supervision settings.}
\textbf{(A) Applicability supervision:} each trace contains an initial state and an action sequence of applicable and inapplicable actions, without any subsequent states. In this setting, extracting a \strips model is possible only for the LS Transformer.
\textbf{(B) Final-state supervision:} each trace contains an initial state, a sequence of applicable actions, and a final state, while intermediate states remain unobserved. A lifted \strips model can be extracted from either architecture through sequential state reconstruction.
}
%\caption{ 
