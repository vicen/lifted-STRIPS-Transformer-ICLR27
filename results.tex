...
\begin{figure}[t]
    \centering
    \includegraphics[width=.9\columnwidth]{imgs/gen_plot_blocksworld.jpg}
\caption{\textbf{Generalization in \bw.}
Test accuracy for the LS and LB Transformers when varying
\textbf{(a)} the number of objects up to $10\times$ the training maximum, with trace length fixed at $D=50$, and
\textbf{(b)} trace length up to $10\times$ the training maximum, with the number of objects fixed at its largest training value.
For LS, we use the shared predicate configuration from Tables~\ref{tab:main-results} and~\ref{tab:main-results-cont} and report both supervision settings.
To assess generalization after successful fitting, we show only the seed with the highest training accuracy.
Each point uses $10^4$ traces, and the $y$-axis starts at $0.6$.
Results for the remaining domains are in Appendix~\ref{app:gen}.}
\label{fig:bw-gen}
\iffalse    
    \caption{\textbf{Generalization in \bw.}
    Test accuracy for the \strips and LB Transformers when generalizing to \textbf{(a)} larger numbers of objects, up to $10\times$ the training size, while keeping trace length to the training value of $D=50$, and
...
% > We remove the lifted strips fragment due to space. They all appear in the appendix.
\iffalse
\begingroup
\begin{figure*}[t]
\centering
...
Tables~\ref{tab:main-results} and~\ref{tab:main-results-cont} show that both softmax baselines perform poorly across domains, with low training and test accuracy and inaccurate extracted \strips models.
Sinusoidal yields zero or near-zero planning accuracy in four of five domains, while RoPE does so in three.
In contrast, LB achieves high training and test accuracy in every domain and perfect planning in four.
We attribute this gap to LB's order-based recency bias from stick-breaking attention and strict future masking without positional encodings.
This matches \strips semantics, where applicability depends on the most recent preceding action affecting each precondition, regardless of distance.

\paragraph{RQ2: Symbolic structure and supervision.}
Under final-state supervision, both architectures perform strongly, while LS achieves perfect accuracy across all reported metrics for every seed and domain.
We attribute its advantage to the direct symbolic parameterization of preconditions and effects.
Applicability-supervised LS shows high variance, but in every domain at least one predicate configuration achieves perfect accuracy across all metrics for its best seed.
Thus, both supervision settings can recover the underlying model, although final-state supervision is easier to optimize.
Successful runs with the Shared configuration also show that LS need not know the exact number of ground-truth predicates.

\paragraph{RQ3: \strips model extraction and planning.}
Appendix~\ref{app:strips-models} contains all extracted models, which recover the ground-truth model or an equivalent one in almost all cases.
Two deviations deserve note.
First, applicability-supervised LS learns extra \texttt{move} effects in \np that preserve the dynamics but introduce irrelevant atoms
(Listing~\ref{lst:npuzzle_full}).
Mimir then exhausts its search budget on two problems, reducing planning accuracy to $0.98$.
Second, LB omits \texttt{at} preconditions from the \texttt{drive}, \texttt{fly}, and \texttt{load} actions in \lgt
(Listings~\ref{lst:logistics_full}--\ref{lst:logistics_full_continuation}), likely because approximately $95\%$ of \texttt{at} atoms in the final states are false.
LS recovers the correct \lgt model under the same supervision.
Except for the search-budget case above, correctly recovered models yield perfect planning, while their explicit form reveals the precise errors in incorrect models.

\paragraph{RQ4: Generalization to longer traces and more objects.}
Figure~\ref{fig:bw-gen} reports results for \bw, with the remaining domains in Appendix~\ref{app:gen}.
We vary trace length and object count separately up to $10\times$ their training maxima.
LS remains perfect in both dimensions under both supervision settings.
LB also generalizes strongly, with its largest losses under object scaling: accuracy falls from $99.5\%$ to $96.6\%$ in \bw and from $95.6\%$ to approximately $80\%$ in \lgt, where it plateaus for the largest instances.
...
\iffalse
    ...
    \paragraph{RQ3: \strips model extraction and planning.}
    Appendix \ref{app:strips-models} contains the \strips models extracted from the transformers in each domain.
